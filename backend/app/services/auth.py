...
class AuthService:
    ...
    def start_url(self) -> str:
        state = sign_oauth_state(self._settings.wechat_oauth_state_secret)
    ...
    def complete_login(self, code: str | None, state: str | None) -> LoginResult:
        ...
        if not verify_oauth_state(self._settings.wechat_oauth_state_secret, state):
        ...
        self._db.commit()
        return LoginResult(session=session, user=user)
